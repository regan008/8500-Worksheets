---
title: 'Worksheet 4: Data Manipulation'
author: ""
date: ""
---
...
Now that you've learned the basic dplyr verbs, string manipulation, handling missing data, and factors, let's combine these skills to answer historical questions.

(@) Using the BostonWomenVoters dataset, create a cleaned and filtered dataset that:
- Removes any rows with missing Age or Ward data
- Cleans the Occupation column by trimming whitespace
- Converts Occupation to a factor
- Creates a new column called `age_group` that categorizes voters as "Under 30", "30-50", or "Over 50"
- Filters to only include voters in Ward 1
- Arranges the results by age (oldest first)

Write your code below and use comments to explain each step:
```{r}
