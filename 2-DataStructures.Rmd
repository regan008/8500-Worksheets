...
bostonvoter <- function (occupation) {
  ...
  bostonvoter.byoccupation <- data.frame()
  for (i in 1:length(BostonWomenVoters$Occupation)) {
    if (BostonWomenVoters$Occupation[i] == occupation) {
      bostonvoter.byoccupation <-  rbind(bostonvoter.byoccupation, BostonWomenVoters[i,])
    } else {
      next 
    } 
  }
  return(bostonvoter.byoccupation)
}

housewifedata <- bostonvoter("Housewife")
